def add_to_label(scroll_area_contents, text_label, text_to_add, label_type, count= None):
...
def remove_from_label(scroll_area_contents, text_label):
    reatained_values = text_label.text().split(",")[:-1]
    text_label.setText(", ".join(reatained_values))
    text_label.adjustSize()
    scroll_area_contents.setFixedWidth(text_label.width())